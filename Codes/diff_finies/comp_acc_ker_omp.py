...
def gflops(d):
    ...
    for i in range(len(d["time"])):
        r.append(12 * 1000 * (d["m_size"][i] ** 2)/ (d["time"][i] * (10 ** 9)))
    return r
...
# plt.legend()
# plt.title("Comparaison des temps d'exécution d'implémentations des différences finies")
...
plt.legend()
plt.title("Comparaison des performances en GFLOPS d'implémentations des différences finies")
plt.xlabel("Taille de la grille")
