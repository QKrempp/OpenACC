...
#pragma acc enter data pcopyin(A[:A_l * A_c * sizeof(double)]) pcreate(A_t[:A_c * A_l * sizeof(double)])
	}

	for(int i = 0; i < num_blocks; i++){
	acc_set_device_num(i%2, acc_device_nvidia);
#pragma acc parallel loop gang num_gangs(256) num_workers(32) async(i)
...
#pragma acc update self(A_t[i * block_c_size * A_c:A_l * block_c_size]) async(i)
	}
	for(int gpu = 0; gpu < 2; gpu++){
	acc_set_device_num(gpu, acc_device_nvidia);
#pragma acc wait
#pragma acc exit data delete(A[:A_l * A_c], A_t[A_c * A_l])
	}
	return A_t;
}
