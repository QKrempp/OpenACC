...
#pragma acc data pcopyin(A[0:A_s], B[0:B_s], C[0:C_s]) pcreate(B_t[0:B_s])
{
...
#pragma acc loop worker 
	...
	for(int block = 0; block < num_blocks; block++){
		int start_l = block * C_l / num_blocks;
		int stop_l = (block + 1) * C_l / num_blocks;
#pragma acc parallel loop gang num_workers(32) vector_length(32) async(block)
		for(int i = start_l; i < stop_l; i++){
#pragma acc loop worker
			for(int j = 0; j < C_c; j++){
				C[i * C_c + j] *= beta;
				float c = 0;
...
#pragma acc update self(C[start_l * C_c: C_s / num_blocks]) async(block)
	}	
#pragma acc wait
}
	return C;
